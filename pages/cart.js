...
function CartScreen() {
  ...
  const { data } = await axios.get(`/api/products/${item._id}`);
    if (data.countInStock < quantity) {
      toast.error('Sorry. Product is out of stock',{duration: 4000,position: "top-center"});
      return;
    }
  ...
  return (
    ...
    <Box  sx={{transform:"translateY(50px)",paddig:'5rem 5rem'}}>
      ...
      {cartItems.length === 0 ? (
        ...
        <Grid container spacing={1} sx={{padding:'2rem 5rem 2rem 5rem'}}>
